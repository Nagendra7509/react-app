...
const CartContainer = styled.div `
    ${tw `flex  fixed  top-0 right-0 z-10 w-1/3 overflow-x-hidden h-full flex-1`}`;
...
const BillAndPayments = styled.div `
    ${tw `absolute bottom-0 right-0 p-4 bg-gray-800 shadow-inner w-11/12`}`;
const CartIconDiv = styled.div `
    ${tw `bg-black text-white fixed top-0 right-0`}
    height:50px;
     width:50px`;
